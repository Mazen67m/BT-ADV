...
import { insertBtsItem, updateBtsItem, deleteBtsItem } from "@/actions/bts";
import type { BtsItemInsert, BtsItemUpdate } from "@/actions/bts";
            ...
            return (
              <div key={item.id} className="bg-surface rounded-2xl border border-border-input overflow-hidden group relative break-inside-avoid">
                <div className="w-full bg-[#020F1C] relative">
                  ...
                  {/* Media type badge */}
                  <div className="absolute top-2 left-2 bg-navy text-white/80 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider flex items-center gap-1.5">
                    {item.media_type === 'video' ? (
                      <><Video className="w-3 h-3 text-yellow" /> Video</>
                    ) : (
                      <><ImageIcon className="w-3 h-3 text-yellow" /> Image</>
                    )}
                  </div>
                    ...
                    {isLoading ? (
                      ...
                      <>
                        <button
                          ...
                          className="w-10 h-10 rounded-full bg-yellow text-navy hover:scale-110 flex items-center justify-center transition-transform"
                        >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              <div className="flex items-center justify-between p-6 border-b border-border-input bg-surface-deep">
                <h3 id="bts-modal-title" className="text-xl font-bold text-white">
