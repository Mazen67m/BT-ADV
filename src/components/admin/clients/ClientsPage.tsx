...
import { insertClient, updateClient, deleteClient } from "@/actions/clients";
import type { ClientInsert, ClientUpdate } from "@/actions/clients";
  ...
  return (
    <div className="space-y-8">
      ...
      {sortedClients.length === 0 ? (
        <div className="text-center py-16 bg-surface rounded-2xl border border-border-input">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {sortedClients.map((client) => {
            ...
            return (
              <div key={client.id} className="bg-surface rounded-2xl border border-border-input p-4 flex flex-col items-center justify-center group relative h-48">
                  ...
                  {isLoading ? (
                    ...
                    <>
                      <button
                        ...
                        className="w-7 h-7 rounded bg-border-input text-white hover:bg-yellow hover:text-navy flex items-center justify-center transition-colors"
                      >
                ...
                <div className="flex-1 flex items-center justify-center w-full mb-3 relative">
                  {client.logo_url ? (
                    <Image src={client.logo_url} alt={client.name} width={200} height={64} className="max-h-16 max-w-[80%] object-contain" />
                  ) : (
                    <ImageIcon className="w-12 h-12 text-border-input" />
                  )}
                  {client.youtube_url && (
                ...
                <div className="w-full text-center mt-auto border-t border-border-input pt-3">
       ...
       <AnimatePresence>
        {isModalOpen && (
           ...
           <motion.div
             ...
             initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-xl bg-surface rounded-2xl overflow-hidden shadow-2xl border-t-4 border-yellow flex flex-col max-h-[90vh]">
             <div className="flex items-center justify-between p-6 border-b border-border-input bg-surface-deep flex-shrink-0">
               <h3 id="clients-modal-title" className="text-xl font-bold text-white">
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-1.5">
                   ...
                   <select
                     ...
                     className="w-full bg-surface-deep text-white/90 border border-border-input rounded-lg px-4 py-2.5 focus:border-yellow focus:outline-none appearance-none"
                   >
               ...
               <div className="space-y-1.5">
                 ...
                 <div className="relative">
                   ...
                   <input
                     ...
                     className="w-full bg-surface-deep text-white border border-border-input rounded-lg pl-10 pr-4 py-2.5 focus:border-yellow focus:outline-none"
                     placeholder="https://youtube.com/watch?v=..."
                   />
                 </div>
               </div>

                <div className="space-y-1.5 pt-2 border-t border-border-input">
                 <label className="text-xs font-bold text-white/50">Sort Order Index</label>
                 <input
                   type="number"
                   value={formData.order_index}
                   onChange={(e) => setFormData({ ...formData, order_index: parseInt(e.target.value) || 0 })}
                   className="w-24 bg-surface-deep text-white border border-border-input rounded-lg px-3 py-1.5 focus:border-yellow focus:outline-none text-sm block"
                 />
               </div>
             </div>
